...
int main() {
    ...
    char* name_list = malloc(sizeof(char) * MAX_NAME * count);

    for (int i = 0; i < count; i++) {
        fgets(name_list + i * MAX_NAME, MAX_NAME, fp);
        *(name_list + i * MAX_NAME + strcspn(name_list + i * MAX_NAME, "\n")) = '\0';
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        (*(arr + i)).name = malloc(sizeof(char) * (strlen(name_list + idx * MAX_NAME) + 1));

        strcpy((*(arr + i)).name, name_list + idx * MAX_NAME);
        (*(arr + i)).id = i + 1;
